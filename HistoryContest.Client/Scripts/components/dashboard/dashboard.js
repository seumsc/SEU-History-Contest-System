...
function setGENERAL() {
    ...
    for (var statIteratorIndex = 0; statIteratorIndex < STAT.length; statIteratorIndex++) {
        ...
        generalContent += '<tr><td class = "departmentID">' + STAT[statIteratorIndex].departmentID+" "+shortForm[STAT[statIteratorIndex].departmentID]
            + '</td><td>' + average
            + '</td><td>' + completion
            + '</td><td>' + proportionA
            + '</td><td>' + proportionB
...
exports.sort = function () {
    $("#done-head .score").click(function () {
        console.log("click");
        if (cnt % 3 == 0) {
            $("#Scoresort").hide();
