...
exports.set = function(){
    ...
    for (questionsIteratorIndex = 0; questionsIteratorIndex < init.questions.length; questionsIteratorIndex++) {
        content += '<section class="panel" id="q'+(questionsIteratorIndex + 1)+'"><div class="inner columns" >'//题目id，为网页内链接表示（实现按钮跳转到下一题）
    ...
    //    if(questionsIteratorIndex+1!= init.questions.length){
            ...
            content+='<section id="picq'+(questionsIteratorIndex + 1)+'" class="panel spotlight large right" ><div class="image  tinted" data-position="top left"><img src="/dist/Images/background'+(questionsIteratorIndex%9)+'.jpg" alt="" /></div></section>'
    //    }
    }

    //生成判断题html
     for (; questionsIteratorIndex < init.questions.length+init.TFs.length; questionsIteratorIndex++) {//延续之前的索引
        content += '<section class="panel" id="q'+(questionsIteratorIndex + 1)+'"><div class="inner columns">'
...
exports.answercard= function(){
    ...
    for(var i=0;i<init.questions.length+init.TFs.length;i++){
        contentFooter +='<a id="jumptoq'+(i+1)+'" id="question'+(i+1)+'" class="questionId">'+(i+1)+"</a>";
